...
export function DataProvider({ children }) {
  ...
  async function _bootstrap() {
    ...
    fbStartListening(db, {
      onStudentsUpdate: incoming => {
        // Preserve local photos (not stored in Firebase)
        setStudents(prev => {
          const photoMap = {}
          prev.forEach(s => { if (s.photo) photoMap[s.id] = s.photo })
          return incoming.map(s => photoMap[s.id] ? { ...s, photo: photoMap[s.id] } : s)
        })
      },
