...
// ── pdfHeader ─────────────────────────────────────────────────────────────
/**
 * Back-compat shim: the branded header now lives in reportTemplate.js
 * (drawReportHeader) so every export shares one logo/school/department/address
 * path. Kept so existing callers (reportCard.js) keep working.
 * @returns {number} y = 34
 */
export function pdfHeader(doc, title, subtitle, cls, accent = REPORT_ACCENTS.grades) {
  return drawReportHeader(doc, { title, subtitle, accent })
}
...
export async function buildGradesPDFDoc(data, students, classes) {
  ...
  // Footer
  // Signatures + branded footer (shared engine)
...
export async function buildAttendancePDFDoc(data, students, classes) {
  ...
  const subtitle = `${cls.name || cls.id}  ·  Section: ${cls.section || '-'}  ·  S.Y. ${cls.sy || '-'}  ·  Exported: ${fmtDate()}`
  let y = pdfHeader(doc, 'Attendance Summary', subtitle, cls, REPORT_ACCENTS.attendance)
...
export async function buildStudentPDFDoc(s, classes, students, eqScale = DEFAULT_EQ_SCALE, opts = {}) {
  ...
  // ── Signatures + branded footer (shared engine) ───────────────────────
  drawSignatures(doc, doc.lastAutoTable.finalY, ['Prepared by', 'Verified by', 'Received by'])
  drawReportFooter(doc, {
    note: `This document was generated via AcadFlow on ${fmtDate()}. For official purposes, please request a signed copy from the registrar.`,
  })
...
// ── Quiz / Activities score-matrix report (per class) ──────────────────────
/**
 * Landscape A4 score matrix (students x quizzes|activities) with an Average (%)
 * column. Shares the branded engine header/footer/signatures.
...
async function buildScoreMatrixPDFDoc(data, { title, accent, fileBase }) {
  ...
  doc.autoTable({
    ...
    didParseCell(hookData) {
      const { row, column, cell } = hookData
      if (row.section === 'head') return
      if (column.index === lastIdx) {
        const val = parseFloat(cell.raw)
        if (!isNaN(val)) {
          if (val >= 75) { cell.styles.fillColor = [220, 252, 231]; cell.styles.textColor = [22, 101, 52] }
          else if (val >= 50) { cell.styles.fillColor = [254, 249, 195]; cell.styles.textColor = [133, 77, 14] }
          else { cell.styles.fillColor = [254, 226, 226]; cell.styles.textColor = [153, 27, 27] }
        }
      }
      if (row.index === rows.length) { cell.styles.fontStyle = 'bold'; cell.styles.fillColor = [237, 233, 254] }
    },
    margin: { left: 8, right: 8 },
  })
  ...
  const safeSection = safeFileName(cls.section || cls.name || cls.id)
  const datePart = new Date().toISOString().slice(0, 10)
  doc.save(`${fileBase}_${safeSection}_${datePart}.pdf`)
}

export function buildQuizPDFDoc(data) {
  return buildScoreMatrixPDFDoc(data, { title: 'Quiz Report', accent: REPORT_ACCENTS.quiz, fileBase: 'QuizReport' })
}
export function buildActivitiesPDFDoc(data) {
  return buildScoreMatrixPDFDoc(data, { title: 'Activities Report', accent: REPORT_ACCENTS.activities, fileBase: 'ActivitiesReport' })
}
