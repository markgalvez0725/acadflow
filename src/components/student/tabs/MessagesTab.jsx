...
function getStudentMessages(messages, s) {
  ...
  return messages.filter(m =>
    ...
    (m.type === 'announcement' && m.classId && m.classId === s.classId)
  ).sort((a, b) => b.ts - a.ts)
}
...
export default function MessagesTab({ student: s, messages }) {
  ...
  async function markRead(msgIds) {
    ...
    for (const id of msgIds) {
      ...
      const newRead = [...(m.read || []), s.id]
      const newReadAt = { ...(m.readAt || {}), [s.id]: now }
  ...
  function openConversation() {
    ...
    const unreadIds = directMsgs.filter(m => m.from !== s.id && !(Array.isArray(m.read) && m.read.includes(s.id))).map(m => m.id)
    markRead(unreadIds)
