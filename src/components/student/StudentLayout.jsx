...
export default function StudentLayout() {
  ...
  useEffect(() => {
    if (student?.forceChangePassword) { setForcePassIsForced(true); setForcePassOpen(true) }
  }, [student?.forceChangePassword])
  ...
  const unreadMsgCount = messages.filter(m => {
    ...
    return isVisible && !(Array.isArray(m.read) && m.read.includes(id))
  }).length
  ...
  return (
    <div className="student-layout" id="student-portal">
      ...
      {forcePassOpen && (
        <Suspense fallback={null}>
          <ForceChangePasswordModal student={student} onClose={() => setForcePassOpen(false)} />
        </Suspense>
      )}
      <Suspense fallback={null}>
        <StudentActionSheet
          ...
          onChangePassword={() => setForcePassOpen(true)}
