...
// Upgrade path for the meeting Smart Recap: when GROQ_API_KEY is set, produce
// an abstractive multi-language summary of the class transcript; when unset,
// return 501 so the client falls back to the on-device extractive recap
// (utils/meetingRecap.js). Mirrors generate-quiz.js.
export default async function handler(req, res) {
  ...
  const prompt = [
    ...
    'Sections, each with an <h4> heading followed by a <ul> (or <p> for Participation):',
  ...
  // Strip any accidental code fences / stray markdown the model may add.
